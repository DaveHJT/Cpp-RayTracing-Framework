	...
	if (inter.hit) {
		...
		if (maxHit > 0) {
			...
			if (inter.mat->m_reflectivity != 0 ) {
				...
				// float fudge = 0.00f;
				Ray reflectRay = Ray(offsetPos, reflectDir, ray.time);
			...
			if (inter.mat->m_refraction != 0) {
				...
				if (internalRefraction >= 0) {
					...
					Ray refractRay = Ray(offsetPos, t, ray.time);
...
inline vec3 getRandomInDisk() {
    vec3 pos;
    do {
        pos = 2.0f * vec3(rand(), rand(), 0) - vec3(1, 1, 0);
    } while (dot(p,p) >= 1.0);
    return p;
}
			...
			for (uint x = 0; x < nx; ++x) {
				...
				vec3 color = vec3(0);
				 ...
				 for(float i = 0; i < sample; i++){
						...
						ray.direction = vec3(normalize(pworld * vec4((float)x + rand() % 100 / 50.0f - 1.2f, (float)y + rand() % 100 / 50.0f - 1.2f, 0.0f, 1.0f) - lookFrom));
