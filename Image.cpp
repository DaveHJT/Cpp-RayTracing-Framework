...
static uint clamp(uint x, uint a, uint b)
{
	return x < a ? a : (x > b ? b : x);
}
vec3 Image::getColor(uint x, uint y) {
  x = clamp(x, 0, m_width);
  y = clamp(y, 0, m_height);
  int idx = m_colorComponents * (m_width * y + x);
  vec3 color = vec3((float)m_data[idx], (float)m_data[idx + 1], (float)m_data[idx + 2]);
  return color;
}
//---------------------------------------------------------------------------------------
...
bool Image::loadPng(const std::string & filename) {
    ...
    unsigned error = lodepng::decode(image, m_width, m_height, png);
