...
static float AIR_REFRACTIVE_IDX = 1.0f;
static float GLASS_REFRACTIVE_IDX = 1.4f;
static float WATER_REFRACTIVE_IDX = 1.3f;

static int duration = 5;
static int fps = 24;
static int sample = 18;
static int state = 0;
static int maxHitTimes = 2;

static float d[] = {10.0f, 2.0f, 5.0f, 5.0f};
static float lastTime = 0;
static float radius = 80.0f;

static float aperture = 15.0f;
	...
	if (inter.hit)
	{
		...
		for (const Light *light : lights)
		{
			...
			if (!shadowInter.hit)
			{
				...
				if (n_dot_h == 0)
					cout << "pos: " << to_string(inter.position) << " normal: " << to_string(inter.normal) << endl;
				if (n_dot_h < 0)
					n_dot_h = 0;
	...
	const std::list<Light *> &lights)
{
	...
	for (string name : ballNames)
	{
		...
		if (ballNodes.back()->m_name == "glassball")
			ballNodes.back()->velocity = vec3(-400, 0, 0) / fps;
	}
