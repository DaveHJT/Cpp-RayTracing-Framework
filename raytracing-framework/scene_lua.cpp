...
int gr_nh_sphere_cmd(lua_State* L)
{
  ...
  data->node = new GeometryNode(name, new NonhierSphere(vec3(), radius));
  data->node->set_transform(translate(mat4(), pos));
...
int gr_nh_box_cmd(lua_State* L)
{
  ...
  data->node = new GeometryNode(name, new NonhierBox(pos, size, mesh));
...
int gr_mesh_cmd(lua_State* L)
{
  ...
  std::cout << "map: "<< mesh_map.size() << sfname << std::endl;
  for (std::map<std::string,Mesh*>::iterator i = mesh_map.begin(); i != mesh_map.end(); i++) {
